...
//!
//! 工具实现（read/write/edit/ls/mkdir/rm/grep + 写前备份/回滚）已抽到
//! `pi-host-tools` crate（2026-09-19，spike/quickjs-agent）。本文件保留同名
//! 转发函数，签名与错误文案逐字不变；全局路径配置（`WORKSPACE_DIR` /
//! `DATA_DIR`）仍是本模块的 OnceLock，构造实例时传入。
...
/// 用自己的 root 做同一个判定。实现已移到 `pi-host-tools`，这里保留同名转发。
...
///
/// 实现已移到 `pi-host-tools`（跨宿主复用，见 `docs/POCKET-PI-NOTES.md`）：
/// 这里只做转发，行为与抽取前逐字一致（含错误文案）。
...
fn tools() -> Result<pi_host_tools::HostTools, String> {
...
/// 会话 fs 通道（pi `JsonlSessionRepo` 的 12 个方法）。
///
/// 实现已移到 `pi-host-tools::sessions_fs`（跨宿主复用，见
/// `docs/POCKET-PI-NOTES.md`）；这里只把 sessions 根传进去。
fn fs_op(payload: &serde_json::Value) -> serde_json::Value {
    match SESSIONS_DIR.get() {
        Some(root) => pi_host_tools::fs_op(std::path::Path::new(root), payload),
        None => pi_host_tools::fs_err("unknown", "sessions dir not configured".into()),
    }
}
