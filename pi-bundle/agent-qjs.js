...
async function restoreLatestSession() {
  ...
  if (!metas.length) {
    outbox.push({ type: "session_restored", found: 0 });
    return;
  }
...
function boot(configJson) {
  ...
  agent.subscribe((event) => {
    // **原样转发**给宿主，由 Rust 侧投成 pi-agent-event 给 UI ——
    // UI（src/lib/events.ts）认的是 pi-agent-core 的原始事件形状
    // （message_start/update/end、turn_start/end、tool_execution_*、agent_start/end…），
    // 所以这里不做精简。spike 那份 CLI 版本精简过，是因为它自己打印。
    outbox.push(event);
...
function prompt(text) {
  ...
  void autoCompactIfNeeded()
    ...
    .finally(() => {
      // ⚠️ 成功路径**不**补 agent_end —— pi-agent-core 自己会发（现在原样转发给 UI），
      // 再补一条就成了重复事件（第一次跑集成测试就看到两个 agent_end）。只有失败要报。
      agent.prompt(text).catch((error) =>
        outbox.push({ type: "agent_error", message: String(error) }),
      );
    });
}
...
globalThis.__spike = {
  ...
  draftPlan,
  askByTheWay,
  // MCP 热重连（对应 bun 路线的 __pi_mcp_reconnect）
  mcpReconnect: () => {
